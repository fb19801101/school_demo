...
function checkTea(form){
	if(form.teiName.value==""){
		alert("请输入教师姓名!");form.teiName.focus();return false;
	}
	if(form.teiAge.value==""||isNaN(form.teiAge.value)){
		alert("请以数字格式输入教师年龄!");form.teiAge.focus();return false;
	}
	if(form.teiCsrq.value==""||CheckDate(form.teiCsrq.value)){
		alert("请输入出生日期（如：1981-02-08）!");form.teiCsrq.focus();return false;
	}
	if(form.teiSfzh.value==""||isNaN(form.teiSfzh.value)){
		alert("请以数字格式输入身份证号码!");form.teiSfzh.focus();return false;
	}
	if(form.teiLxdh.value==""||isNaN(form.teiLxdh.value)){
		alert("请以数字格式输入联系电话号码!");form.teiLxdh.focus();return false;
	}
	if(form.teiGzrq.value==""||CheckDate(form.teiGzrq.value)){
		alert("请输入工作日期（如：1981-02-08）!");form.teiGzrq.focus();return false;
	}
}
function checkCourse(form){
	if(form.teiId.value==""||isNaN(form.teiId.value)){
		alert("请以数字格式输入教师编号！");form.teiId.focus();return false;
	}
	if(form.aceRzrq.value==""||CheckDate(form.aceRzrq.value)){
		alert("请输入教师任课日期（如：1981-02-08）！");form.aceRzrq.focus();return false;
	}
}
function checkCharge(form){
	...
	if(form.acsRzrq.value==""||CheckDate(form.acsRzrq.value)){
		alert("请输入教师任职日期（如：1981-02-08）！");form.acsRzrq.focus();return false;
	}
}
